...
public class PizzaBlockEntity extends BlockEntity {
  ...
  protected void collectImplicitComponents(DataComponentMap.Builder componentMapBuilder) {
    ...
    componentMapBuilder.set(NSDataComponents.TOPPINGS, toppings);
  }

  @Override
  public void saveAdditional(CompoundTag nbt, HolderLookup.Provider registryLookup) {
    super.saveAdditional(nbt, registryLookup);
    nbt.put("toppings", (Tag)NSDataComponents.TOPPINGS.codec().encodeStart(NbtOps.INSTANCE, this.toppings).getOrThrow());
  }
  ...
  public void loadAdditional(CompoundTag nbt, HolderLookup.Provider registryLookup) {
    ...
    if (nbt.contains("toppings")) {
      NSDataComponents.TOPPINGS.codec().parse(NbtOps.INSTANCE, nbt.get("toppings")).resultOrPartial().ifPresent((list) -> toppings.addAll(list));
    }
  }
